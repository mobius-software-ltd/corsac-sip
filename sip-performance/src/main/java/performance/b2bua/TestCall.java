...
public class TestCall {
	...
	private Request createRequest(Request origRequest) throws SipException {
		...
		if (outgoingDialog == null || outgoingDialog.getState() == null) {
			...
			for (Iterator<Route> it = (Iterator<Route>) outgoingDialog.getRouteSet(); it.hasNext();) {
				Route route = it.next();				
				routeList.add(route);								
			}
	...
	private void forwardResponse(Response receivedResponse) throws SipException, InvalidArgumentException {
		...
		// copy headers 
		ListIterator<String> lit = (ListIterator<String>) receivedResponse.getHeaderNames();
		...
		while (lit.hasNext()) {
			...
			if (TestCall.getHeadersToOmmitOnResponseCopy().contains(headerName)) {
				...
				headersIterator = (ListIterator<Header>) receivedResponse.getHeaders(headerName);
