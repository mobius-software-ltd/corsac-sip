...
public abstract class TestHarness extends TestCase {
...
//                    .getResourceAsStream("tck.properties"));
            ...
            Set<String> props = tckProperties.stringPropertyNames();
            ...
            if (lf != null) {            	
            	configuration.addAppender(FileAppender.newBuilder().setName("TCKOutput").withFileName(logFileName).build());
            } else {
            	configuration.addAppender(FileAppender.newBuilder().setName("TCKOutput").withFileName("target/logs/tckoutput.txt").build());                        
            }
        } catch (Exception ex) {
