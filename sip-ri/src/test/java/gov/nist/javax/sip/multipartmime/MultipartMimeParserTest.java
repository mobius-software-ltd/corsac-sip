...
public class MultipartMimeParserTest extends TestCase {
	...
	private static String smsInfoString = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
			+ "<short-message-info>\n"
			+ " <From>sip:user1_public1@home1.net</From>\n"
			+ "</short-messsage-info>";

	private static byte[] binaryData = new byte[256];
	...
	public void testCreateMultipartMimeContent() throws Exception {
		...
		MultipartMimeContent mmc = messageFactory.createMultipartMimeContent(cth, type, subtype, content,
				((SIPMessage) sipMessage).getCharset());
		Iterator<Content> contentIterator = mmc.getContents();

		int i = 0;
		while (contentIterator.hasNext()) {
			Content contentImpl = contentIterator.next();
			assertEquals("Content must match", content[i],
					new String(contentImpl.getContent(), ((SIPMessage) sipMessage).getCharset()));
		...
		((SIPMessage) sipMessage).setContent(mmc.getEncodedValue(), cth);
		...
		while (contentIterator.hasNext()) {
			Content contentImpl = contentIterator.next();
			assertEquals("Content must match", content[i], new String(contentImpl.getContent()));
			assertEquals("Content Type match", type[i], contentImpl.getContentTypeHeader().getContentType());
			assertEquals("Content subtype match", subtype[i], contentImpl.getContentTypeHeader().getContentSubType());
			i++;
		}
		...
		while (contents2.hasNext()) {
			...
			assertTrue("contents must match", Arrays.equals(c1.getContent(), c2.getContent()));
	...
	public void testSms() throws Exception {
		SIPMessage sipMessage = (SIPMessage) messageFactory.createRequest(messageString);
		ContentTypeHeader cth = headerFactory.createContentTypeHeader("multipart", "mixed");
		cth.setParameter("boundary", "outer");
		MultipartMimeContent mmc = new MultipartMimeContentImpl(cth, sipMessage.getCharset());
		
		ContentImpl textContent = new ContentImpl(smsInfoString);
		textContent.setContentTypeHeader(headerFactory.createContentTypeHeader("application", "vnd.3gpp.sms+xml"));		
		mmc.add(textContent);
		
		ContentImpl binaryContent = new ContentImpl(binaryData);
		binaryContent.setContentTypeHeader(headerFactory.createContentTypeHeader("application", "vnd.3gpp.sms"));		
		mmc.add(binaryContent);
		
		sipMessage.setContent(mmc.getEncodedValue(), cth);
		
		MultipartMimeContent decodedMime=sipMessage.getMultipartMimeContent();
		assertNotNull(decodedMime);
		assertEquals(decodedMime.getContentCount(),2);
		
		Iterator<Content> iterator = decodedMime.getContents();
		Content firstContent = iterator.next();
		assertNotNull(firstContent.getContentTypeHeader());
		assertEquals(firstContent.getContentTypeHeader().getContentType(),"application");
		assertEquals(firstContent.getContentTypeHeader().getContentSubType(),"vnd.3gpp.sms+xml");
		assertEquals(new String(firstContent.getContent(),sipMessage.getCharset()), smsInfoString);	
		
		Content secondContent = iterator.next();
		assertNotNull(secondContent.getContentTypeHeader());
		assertEquals(secondContent.getContentTypeHeader().getContentType(),"application");
		assertEquals(secondContent.getContentTypeHeader().getContentSubType(),"vnd.3gpp.sms");
		byte[] decodedData = binaryContent.getContent();
		assertArrayEquals(decodedData, binaryData);
	}
...
//    byte[] content = toByteArray(in);
	...
	public void testMultiPartMimeMarshallingAndUnMarshallingWithANonMultiPartBodyWithAnEmptyLine() throws Exception {
		...
		request.setContent(multipartMimeContent.getContents().next().getContent(), contentType);
		MultipartMimeContent multipartMimeContent2 = request.getMultipartMimeContent();
		checkSimpleBody(multipartMimeContent2);
	}

	public void testMultiPartMimeMarshallingAndUnMarshallingWithANonMultiPartBodyWithAnEmptyLineWithNoLeadingLine()
			throws Exception {
		SIPRequest request = new SIPRequest();
	...
	private void checkMultiPartWithSpaces(MultipartMimeContent multipartMimeContent) {
		...
		assertEquals("application/pkcs7-signature;name=smime.p7s",
				((ContentType) part2.getContentTypeHeader()).getValue());
		assertTrue(part2.getExtensionHeaders().hasNext());
		assertEquals(
				"ghyHhHUujhJhjH77n8HHGTrfvbnj756tbB9HG4VQpfyF467GhIGfHfYT6\n"
						+ "4VQpfyF467GhIGfHfYT6jH77n8HHGghyHhHUujhJh756tbB9HGTrfvbnj\n"
						+ "n8HHGTrfvhJhjH776tbB9HG4VQbnj7567GhIGfHfYT6ghyHhHUujpfyF4\n" + "7GhIGfHfYT64VQbnj756\n",
				new String(part2.getContent()));
