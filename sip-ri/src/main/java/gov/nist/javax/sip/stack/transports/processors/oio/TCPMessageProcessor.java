...
import gov.nist.javax.sip.stack.transports.processors.IOHandler;
...
public class TCPMessageProcessor extends ConnectionOrientedMessageProcessor implements Runnable {
    ...
    public TCPMessageProcessor(InetAddress ipAddress, SIPTransactionStack sipStack, int port) {
        ...
        ioHandler = new IOHandler(sipStack);
    }
        ...
        while (this.isRunning) {
            try {
                synchronized (this) {
                    ...
                    while (sipStack.getMaxConnections() != -1
                            && this.nConnections >= sipStack.getMaxConnections()) {
                        try {
                            this.wait();
