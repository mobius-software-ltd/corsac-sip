...
/**
 * Netty Based Datagram Transport Protocol (UDP) Message Channel to handle SIP
 * Messages
 * 
...
public class NettyDatagramMessageChannel extends MessageChannel implements RawMessageChannel {
	...
	public void sendMessage(SIPMessage sipMessage) throws IOException, MessageTooLongException {
		...
		if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
			logger.logDebug(
					"sending new UDP message to: " + getPeerAddress() + ":" + getPeerPort() + "/" + peerProtocol);
		}
		...
		if (messageProcessor != null) {
			RawMessageChannel messageChannel = (RawMessageChannel) messageProcessor
					.createMessageChannel(this.getPeerInetAddress(), this.getPeerPort());
	...
	public void sendMessage(byte[] message, InetAddress receiverAddress, int receiverPort, boolean reconnectFlag)
		...
		if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
			logger.logDebug("sending new UDP message to: " + receiverAddress + ":" + receiverPort + " msg: "
					+ new String(message));
		}
	...
	public void processMessage(SIPMessage sipMessage) throws Exception {
		...
		if (sipMessage.getFrom() == null || sipMessage.getTo() == null || sipMessage.getCallId() == null
			...
			if (logger.isLoggingEnabled()) {
				...
				logger.logError(">>> Dropped Bad Msg " + "From = " + sipMessage.getFrom() + "To = " + sipMessage.getTo()
						+ "CallId = " + sipMessage.getCallId() + "CSeq = " + sipMessage.getCSeq() + "Via = "
		...
		if (sipMessage instanceof SIPRequest) {
			...
			// if it is too long.
			if (sipStack.getMaxUdpMessageSize() > 0 && sipMessage.getSize() + (sipMessage.getContentLength() == null ? 0
					: sipMessage.getContentLength().getContentLength()) > sipStack.getMaxUdpMessageSize()) {
				SIPResponse sipResponse = ((SIPRequest) sipMessage).createResponse(SIPResponse.MESSAGE_TOO_LARGE);
				byte[] resp = sipResponse.encodeAsBytes(this.getTransport());
				this.sendMessage(resp, sipMessage.getPeerPacketSourceAddress(),
						sipMessage.getPeerPacketSourcePort(), false);
				throw new Exception("Message size exceeded");
			}
			
			String sipVersion = ((SIPRequest) sipMessage).getRequestLine().getSipVersion();
			if (!sipVersion.equals("SIP/2.0")) {
				Response versionNotSupported = ((SIPRequest) sipMessage).createResponse(Response.VERSION_NOT_SUPPORTED,
						"Bad version " + sipVersion);
				this.sendMessage(versionNotSupported.toString().getBytes(), sipMessage.getPeerPacketSourceAddress(),
						sipMessage.getPeerPacketSourcePort(), false);
				return;
			}
			...
			if (!method.equalsIgnoreCase(cseqMethod)) {
				...
				byte[] resp = sipResponse.encodeAsBytes(this.getTransport());
				this.sendMessage(resp, sipMessage.getPeerPacketSourceAddress(), sipMessage.getPeerPacketSourcePort(),
						false);
				return;
		...
		if (sipMessage instanceof SIPRequest) {
			Hop hop = sipStack.getAddressResolver().resolveAddress(topMostVia.getHop());
			...
			try {
				...
				if (sipStack.isPatchRport()) {
					if (!hasRPort && topMostVia.getPort() != peerPacketSourcePort) {
						...
						if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
							logger.logDebug("setting rport since viaPort " + topMostVia.getPort()
									+ " different than peerPacketSourcePort " + peerPacketSourcePort
									+ " so that the response can be routed back");
						}
						hasRPort = true;
					}
				}
				if (hasRPort || !hop.getHost().equals(this.getPeerAddress())) {
	...
	private void processSIPMessage(SIPMessage sipMessage) {
		if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
			logger.logDebug("received new UDP message from: " + this.getPeerAddress() + ":" + this.getPeerPort()
					+ " msg: " + sipMessage);
		}
		...
		if (sipMessage instanceof SIPRequest) {
			...
			if (logger.isLoggingEnabled(ServerLogger.TRACE_MESSAGES)) {
				...
				this.sipStack.getServerLogger().logMessage(sipMessage, this.getPeerHostPort().toString(),
						this.getHost() + ":" + this.myPort, false, receptionTime);

			}
			final ServerRequestInterface sipServerRequest = sipStack.newSIPServerRequest(sipRequest,
			...
			if (logger.isLoggingEnabled(ServerLogger.TRACE_MESSAGES)) {
				...
				this.sipStack.getServerLogger().logMessage(sipResponse, this.getPeerHostPort().toString(),
						this.getHost() + ":" + this.myPort, false, receptionTime);
	...
	public void setPeerProtocol(String peerProtocol) 
	{
