...
import gov.nist.javax.sip.stack.transports.processors.nio.NioTcpMessageProcessor.PendingData;
...
//		}
	...
	protected NioTcpMessageChannel(NioTcpMessageProcessor nioTcpMessageProcessor,
		...
		try {
			...
			this.peerProtocol = nioTcpMessageProcessor.getTransport();
			lastActivityTimeStamp = System.currentTimeMillis();
			super.key = MessageChannel.getKey(peerAddress, peerPort, nioTcpMessageProcessor.getTransport());
	...
	public void sendTCPMessage(byte message[], InetAddress receiverAddress,
		...
		SocketChannel sock = nioHandler.sendBytes(this.messageProcessor
					.getIpAddress(), receiverAddress, receiverPort, this.messageProcessor.getTransport(),
					message, retry, this);
