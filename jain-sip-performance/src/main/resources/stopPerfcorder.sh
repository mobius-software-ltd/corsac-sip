...
rm -rf *.zip
bash ./pc_stop_collect.sh "$@" -f $PERFCORDER_STOP_OPTIONS
###this command is supposed to match just one file
cp -f $TOOLS_DIR/perf*.zip $RESULTS_DIR/perfTest-$RESULT_SUFFIX.zip
### Check for performance regression
./pc_analyse.sh $RESULTS_DIR/perfTest-$RESULT_SUFFIX.zip $ANALYSIS_TRIM_PERCENTAGE > $RESULTS_DIR/PerfCorderAnalysis$RESULT_SUFFIX.xml 2> $RESULTS_DIR/analysis$RESULT_SUFFIX.log
