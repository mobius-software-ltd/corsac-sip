...
//	}
    ...
    public void send(SocketChannel socket, byte[] data) throws IOException  {
        ...
        PendingData pData = new PendingData(MessageChannel.getMessageTxId().get() ,ByteBuffer.wrap(data));
        queue.add(pData);
    ...
    public NioTcpMessageChannel createMessageChannel(NioTcpMessageProcessor nioTcpMessageProcessor, SocketChannel client) throws IOException {
		...
		retval.setMessageProcessor(nioTcpMessageProcessor);
		retval.setClientInputStream(client.socket().getInputStream());
		return retval;
	}    
