...
public class NettyTCPMessageChannel extends MessageChannel implements
	...
	public NettyTCPMessageChannel(InetAddress inetAddress, int port,
			...
			NettyTCPMessageProcessor nettyTCPMessageProcessor) throws IOException {
		if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
	...
	public void processMessage(SIPMessage sipMessage) throws Exception {
		...
		if (sipMessage instanceof SIPRequest) {
			...
			if (!this.isCached && channel != null) { // self routing makes
				...
				((NettyTCPMessageProcessor) this.messageProcessor).cacheMessageChannel(this);
			}
		}
		...
		if (sipMessage instanceof SIPRequest) {
			...
			if (sipServerResponse != null) {
				...
				if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG))
					NettyTCPMessageChannel.logger.logDebug("null sipServerResponse as could not acquire semaphore or the valve dropped the message.");
			}
